 ...
 implements BootImageWriterConstants {
  ...
  public static void main(String[] args) {
    ...
    bootImage = new BootImage(littleEndian, verbose >= 1);
		...
		LinkedList<Object[]> fixupList = new LinkedList<Object[]>();
    ...
    enableObjectAddressRemapper();
		...
		VM_Magic.setBootImageWriterFixupList(fixupList);
    ...
    disableObjectAddressRemapper();
		VM_Magic.setBootImageWriterFixupList(null);
    ...
    //
    // Next, copy the subarch jtoc.
    //
    if (verbose >= 1) say("copying subarch jtoc");
    // Pointer to middle of subarch JTOC
    Address subArchJtocImageAddress = Address.zero();
    try {
      subArchJtocImageAddress = copyToBootImage(VM_SubArchStatics.getSlotsAsIntArray(), false, Address.max(), null);
      if (subArchJtocImageAddress.EQ(OBJECT_NOT_PRESENT)) {
        fail("can't copy subarch jtoc");
      }
    } catch (IllegalAccessException e) {
      fail("can't copy subarch jtoc: "+e);
    }
    Address subArchJtocPtr = subArchJtocImageAddress.plus(VM_SubArchStatics.middleOfTable << LOG_BYTES_IN_INT);
    //if (subArchJtocImageAddress.NE(VM.subArchBootRecord.jtocStart))
    //  fail("mismatch in SubArch JTOC placement "+VM.addressAsHexString(subArchJtocPtr)+" != "+ VM.addressAsHexString(VM.subArchBootRecord.jtocStart));

    //
    // Now, copy all objects reachable from subarch jtoc, replacing each 
		// object id that was generated by object address remapper with the actual
    // bootimage address of that object.
    //
    if (verbose >= 1) say("copying subarch statics");
    try {
      int refSlotSize = VM_SubArchStatics.getReferenceSlotSize();
      for (int i = VM_SubArchStatics.middleOfTable+refSlotSize, n = VM_SubArchStatics.getHighestInUseSlot();
           i <= n;
           i+= refSlotSize) {
        if(!VM_SubArchStatics.isReference(i)) {
          throw new Error("SubArch Static " + i + " of " + n + " isn't reference");
        }

        Offset subArchJtocOff = VM_SubArchStatics.slotAsOffset(i);
        int objCookie;
				objCookie = VM_SubArchStatics.getSlotContentsAsInt(subArchJtocOff);
        ...
        if (verbose >= 2) traceContext.push(jdkObject.getClass().getName(),
                                            getRvmSubArchStaticField(subArchJtocOff) + "");
				
				Address imageAddress = copyToBootImage(jdkObject, false, Address.max(), VM_SubArchStatics.getSlotsAsIntArray());
        if (imageAddress.EQ(OBJECT_NOT_PRESENT)) {
          // object not part of bootimage: install null reference
          if (verbose >= 2) traceContext.traceObjectNotInBootImage();
          bootImage.setNullAddressWord(subArchJtocPtr.plus(subArchJtocOff), false, false);
        } else {
          bootImage.setAddressWord(subArchJtocPtr.plus(subArchJtocOff),
                                   imageAddress.toWord(), false);
        }
        if (verbose >= 2) traceContext.pop();
      }
      ...
      fail("unable to copy statics: "+e);
		...
		for (Object[] tuple: fixupList) {
				Address tibAddr = BootImageMap.getImageAddress(tuple[0], true);
				Integer tibOffset = (Integer)tuple[1];
				tibAddr = tibAddr.plus(tibOffset.intValue());
				if (tuple[2] instanceof Byte) {
						bootImage.setByte(tibAddr, (Byte)tuple[2]);
				} else if (tuple[2] instanceof Short) {
						bootImage.setHalfWord(tibAddr, (Short)tuple[2]);
				} else {
						bootImage.setFullWord(tibAddr, (Integer)tuple[2]);
				}
		}
    ...
    VM_CodeArray startupCode  = (VM_CodeArray) VM_Entrypoints.bootMethod.getCurrentEntryCodeArray(false);
    ...
    bootRecord.greenProcessorsOffset = VM_Entrypoints.greenProcessorsField.getOffset();
    ...
    bootRecord.bootImageRMapEnd   = bootImageRMapAddress.plus(bootImage.getRMapSize());
		bootRecord.subArchBootRecord  = BootImageMap.getImageAddress(VM.subArchBootRecord, true);
    ...
    if (verbose >= 2) {
      ...
      for (int i = FIRST_TYPE_DICTIONARY_INDEX; i < types.length; ++i) {
        ...
        } else if (!type.isInstantiated(false)) {
          say("type referenced but not instantiated: ", type.toString());
        } else if (!type.isInitialized(false)) {
  ...
  public static void createBootImageObjects(Vector<String> typeNames,
    ...
    throws IllegalAccessException {
      ...
      bootRecord.tocRegister = jtocAddress.plus(intArrayType.getInstanceSize(VM_Statics.middleOfTable));
			...
			// Initialize subarch boot record
			VM.initSubArchBootRecord();
      ...
      VM_SubArchStatics.bootImageInstantiationFinished();

      // Do the portion of JNIEnvironment initialization that can be done
      // at bootimage writing time.
      ArchitectureSpecific.VM_CodeArray[] functionTable = BuildJNIFunctionTable.buildTable();
          ...
          if (rvmFieldType.isPrimitiveType()) {
            ...
            if (rvmFieldType.isBooleanType()) {
              ...
              VM_Statics.setSlotContents(rvmFieldOffset,
                       ...
                       rvmFieldType.equals(VM_TypeReference.LocalAddress) ||
  ...
  private static boolean check_addr(Word value) {
    ...
    return !(value.GT(low) && value.LT(high) && 
						!value.EQ(Word.fromIntZeroExtend(32767)) &&
						(value.LT(Word.fromIntZeroExtend(4088)) ||
						 value.GT(Word.fromIntZeroExtend(4096))));
  }
  ...
  private static Word getWordValue(Object addr, String msg, boolean warn) {
    ...
    if (addr instanceof Address) {
				...
				if (!check_addr(value)) {
						try {
								Object ref = BootImageMap.getObject((value.toInt()));
								copyToBootImage(ref, false, Address.max(), null);
								value = BootImageMap.getImageAddress(ref, false).toWord();
						} catch (IllegalAccessException e) {
								VM.sysWriteln("Error getting " + addr);
								VM.sysWriteln("" + e);
								VM.sysFail("incomplete boot image support");
						}
				}
    } else if (addr instanceof LocalAddress) {
				value = ((LocalAddress)addr).toWord();
				if (!check_addr(value)) {
						try {
								Object ref = BootImageMap.getObject((value.toInt()));
								copyToBootImage(ref, false, Address.max(), null);
      ...
      if (jdkType.isArray()) {
        ...
        if (rvmType == VM_Type.CodeArrayType) {
          ...
          return copyMagicArrayToBootImage(backing, rvmType.asArray(), allocOnly, overwriteAddress, parentObject);
				...
				if (rvmType == VM_Type.SubArchCodeArrayType) {
          if (verbose >= 2) depth--;
          org.jikesrvm.SubordinateArchitecture.VM_CodeArray codeArray = (org.jikesrvm.SubordinateArchitecture.VM_CodeArray) jdkObject;
          Object backing = codeArray.getBacking();
          return copyMagicArrayToBootImage(backing, rvmType.asArray(), allocOnly, overwriteAddress, parentObject);
        }
  ...
  private static Address copyMagicArrayToBootImage(Object jdkObject,
    ...
    if (overwriteAddress.isMax()) {
			if (rvmElementType.equals(VM_Type.CodeType) || rvmElementType.equals(VM_Type.SubArchCodeType)) {
    ...
    if (rvmElementType.equals(VM_Type.CodeType)) {
	  ...
	  } else if (rvmElementType.equals(VM_Type.SubArchCodeType)) {
				int[] values = (int[]) jdkObject;
        for (int i = 0; i < arrayCount; ++i)
          bootImage.setFullWord(arrayImageAddress.plus(i << LOG_BYTES_IN_INT), values[i]);
        ...
        for (VM_Method vmCons : klass.getConstructorMethods()) {
          ...
          if (vmConsParams.length == consParams.length) {
            for (int j = 0; j < vmConsParams.length; j++) {
              if (!consParams[j].equals(vmConsParams[j].resolve(false).getClassForType())) {
       ...
       new VM_ObjectAddressRemapper() {
         ...
         public Object addressAsObject(Address address) {
           ...
           return null;
				 ...
				 public <T> LocalAddress objectAsLocalAddress(T jdkObject) {
					 VM.sysWriteln("anonymous VM_ObjectAddressMapper: called localAddressAsObject");
           VM._assert(VM.NOT_REACHED);
           return null;
         }

         public Object localAddressAsObject(LocalAddress address) {
           VM.sysWriteln("anonymous VM_ObjectAddressMapper: called localAddressAsObject");
           VM._assert(VM.NOT_REACHED);
           return null;
         }
   ...
   * @return field name
   */
  private static VM_Field getRvmSubArchStaticField(Offset subArchJtocOff) {
    VM_Type[] types = VM_Type.getTypes();
    for (int i = FIRST_TYPE_DICTIONARY_INDEX; i < types.length; ++i) {
      VM_Type type = types[i];
      if (type == null) continue;
      ...
      if (!type.isResolved(false))
        continue;
      for (VM_Field rvmField : types[i].getStaticFields()) {
        if (rvmField.getSubArchOffset().EQ(subArchJtocOff))
          return rvmField;
      }
    }
    return null;
  ...
  private static VM_CompiledMethod findMethodOfCodeNew(Object code) {
    VM_CompiledMethod[] compiledMethods = VM_CompiledMethods.getCompiledMethods();
    VM.sysWrite("Looking for code - " + code);
      ...
      if (compiledMethod != null &&
          ...
          compiledMethod.getEntryCodeArray() == code) {
					...
					VM.sysWrite("Found - " + compiledMethod.method.getName() + " at " + i + " is subarch " + compiledMethod.isSubArchCompilation());
			}
    }
    return null;
  }
    ...
    for (int i = 0; i < VM_CompiledMethods.numCompiledMethods(); ++i) {
      ...
      if (compiledMethod != null) {
        ...
        if (m != null && compiledMethod.isCompiled()) {
						String category;
						if (compiledMethod.isSubArchCompilation()) {
								category = ".     .          subcode     ";
						} else {
								category = ".     .          code     ";
						}	
	  VM_CodeArray instructions = (VM_CodeArray) compiledMethod.getEntryCodeArray();
          Address code = BootImageMap.getImageAddress(instructions.getBacking(), true);
          out.println(category + VM.addressAsHexString(code) +
  ...
  private static Address getReferenceAddr(Offset jtocOff, boolean fatalIfNotFound) {
    ...
    if (ival != 0) {
      ...
      } else if (jdkObject instanceof SubordinateArchitecture.VM_CodeArray) {
        jdkObject = ((SubordinateArchitecture.VM_CodeArray)jdkObject).getBacking();
