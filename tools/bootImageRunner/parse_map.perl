...
my $wantedCategory = shift(@ARGV); # category wanted (code or subcode
...
while(<MAPFH>) {
  ...
  if (! (($offset, $type, $method, $args, $ret) = m/\.\s+\.\s+$wantedCategory\s+(0x[0-f]+)\s+< \S+, L(\S+); >.(\S+) \((\S*)\)(\S+)/)) {
    if (! ((($offset, $type, $method, $args, $ret) = m/\.\s+\.\s+code\s+(0x[0-f]+)\s+< \S+, L(\S+); >.(\S+) \((\S*)\)(\S+)/) || (($offset, $type, $method, $args, $ret) = m/\.\s+\.\s+subcode\s+(0x[0-f]+)\s+< \S+, L(\S+); >.(\S+) \((\S*)\)(\S+)/))) {
      if (/0x/) {
