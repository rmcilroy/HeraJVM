...
/* Use all available subarch processors by default */
int subArchProcs = -1;
...
#ifdef RVM_SUB_ARCH_CELLSPU
		struct VM_SubArchBootRecord * subArchBoot = 
			(struct VM_SubArchBootRecord *) (bootRecord.subArchBootRecord - 12);
	  if (subArchProcs != 0) {
			initSpuCtrl(subArchProcs, subArchBoot, &bootRecord);
		} else {
			// nothing to do for booting subarch
			subArchBoot->noSubArchProcs = 0;
			subArchBoot->subArchBootComplete = 1;
		}
#endif
