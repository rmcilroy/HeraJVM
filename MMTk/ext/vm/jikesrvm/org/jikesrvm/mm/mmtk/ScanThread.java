...
@Uninterruptible public final class ScanThread implements VM_Constants {
	...
	// TODO - Work out which addresses are LocalAddress and which are Address
  ...
  private LocalAddress ip, fp, prevFp, initialIPLoc, topFrame;
  ...
  public static void scanThread(VM_Thread thread, TraceLocal trace,
    ...
    scanThread(thread, trace, processCodeLocations, gprs, LocalAddress.zero());
  }
  ...
  public static void scanThread(VM_Thread thread, TraceLocal trace,
                                boolean processCodeLocations,
                                Address gprs, LocalAddress topFrame) {
    /* establish ip and fp for the stack to be scanned */
    LocalAddress ip, fp, initialIPLoc;
  ...
  private void startScan(TraceLocal trace,
                         boolean processCodeLocations,
                         VM_Thread thread, Address gprs, LocalAddress ip,
                         LocalAddress fp, LocalAddress initialIPLoc, LocalAddress topFrame) {
    this.trace = trace;
  ...
  private void scanThreadInternal(Address gprs, int verbosity) {
    ...
    if (fp.NE(VM_Magic.addressAsLocalAddress(ArchitectureSpecific.VM_StackframeLayoutConstants.STACKFRAME_SENTINEL_FP))) {
      prevFp = LocalAddress.zero();
      /* At start of loop:
         fp -> frame for method invocation being processed
         ip -> instruction pointer in the method (normally a call site) */
      while (VM_Magic.getCallerFramePointer(fp).NE(VM_Magic.addressAsLocalAddress(ArchitectureSpecific.VM_StackframeLayoutConstants.STACKFRAME_SENTINEL_FP))) {
  ...
  private void getHWExceptionRegisters() {
    VM_Registers hwExReg = thread.getHardwareExceptionRegisters();
    if (processCodeLocations && hwExReg.inuse) {
      LocalAddress ip = hwExReg.ip;
      ...
      LocalAddress ipLoc = hwExReg.getIPLocation();
      if (VM.VerifyAssertions) VM._assert(ip == ipLoc.loadLocalAddress());
   ...
   */
  @Inline
  private void processCodeLocation(ObjectReference code, LocalAddress ipLoc) {
    if (VALIDATE_REFS) {
      LocalAddress ip = ipLoc.loadLocalAddress();
      Offset offset = ip.diff(code.toLocalAddress());
    ...
    trace.processInteriorEdge(code, VM_Magic.localAddressAsAddress(ipLoc), true);
  }
   ...
   * performing the scan.
   */
  private LocalAddress scanFrame(int verbosity) {
  ...
  private void scanFrameForObjects(int verbosity) {
    for (LocalAddress refaddr = iterator.getNextReferenceAddress();
         !refaddr.isZero();
         refaddr = iterator.getNextReferenceAddress()) {
      if (VALIDATE_REFS) checkReference(VM_Magic.localAddressAsAddress(refaddr), verbosity);
      if (verbosity >= 3) dumpRef(VM_Magic.localAddressAsAddress(refaddr), verbosity);
      trace.reportDelayedRootEdge(VM_Magic.localAddressAsAddress(refaddr));
    }
  }
  ...
  private void pushFrameIP(ObjectReference code, int verbosity) {
    if (prevFp.isZero()) {  /* top of stack: IP in thread state */
      ...
      LocalAddress returnAddressLoc = VM_Magic.getReturnAddressLocation(prevFp);
      LocalAddress returnAddress = returnAddressLoc.loadLocalAddress();
      if (verbosity >= 3) {
      ...
      if (!DebugUtil.addrInBootImage(VM_Magic.localAddressAsAddress(returnAddress)))
  ...
  private void scanFrameForCode(ObjectReference code) {
    iterator.reset();
    for (LocalAddress retaddrLoc = iterator.getNextReturnAddressAddress();
         !retaddrLoc.isZero();
  ...
  private void checkJNIBase() {
    if (VM.BuildForAix) {
      ...
      LocalAddress refaddr =  iterator.getNextReferenceAddress();
  ...
  private void checkReference(Address refaddr, int verbosity) {
    ...
    if (!MM_Interface.validRef(ref)) {
      ...
      LocalAddress top_ip = thread.contextRegisters.getInnermostInstructionAddress();
      LocalAddress top_fp = thread.contextRegisters.getInnermostFramePointer();
  ...
  private void printMethodHeader() {
    ...
    if (compiledMethod.isCompiled()) {
        ...
        Log.writeln(ip.diff(codeBase.toLocalAddress()));
    } else {
  ...
  private void dumpStackFrame(int verbosity) {
    ...
    if (VM.BuildForIA32) {
      ...
      end = fp.loadLocalAddress();   // stop at callers fp
    }

    for (LocalAddress loc = start; loc.LT(end); loc = loc.plus(BYTES_IN_ADDRESS)) {
