...
public class Collection extends org.mmtk.vm.Collection implements Constants, VM_Constants {
  ...
  public final void prepareMutator(MutatorContext m) {
    ...
    if (vpStatus == VM_Processor.BLOCKED_IN_NATIVE) {
      ...
      t.contextRegisters.setInnermost(LocalAddress.zero(), t.jniEnv.topJavaFP());
    }
  }
  ...
  public final void prepareCollector(CollectorContext c) {
    ...
    while (true) {
      LocalAddress caller_ip = VM_Magic.getReturnAddress(fp);
      LocalAddress caller_fp = VM_Magic.getCallerFramePointer(fp);
      if (VM_Magic.getCallerFramePointer(caller_fp).EQ(VM_Magic.addressAsLocalAddress(ArchitectureSpecific.VM_StackframeLayoutConstants.STACKFRAME_SENTINEL_FP)))
