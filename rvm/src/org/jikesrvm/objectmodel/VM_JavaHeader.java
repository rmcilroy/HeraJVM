...
public class VM_JavaHeader implements VM_JavaHeaderConstants {
  ...
  public static final Offset TIB_OFFSET = JAVA_HEADER_OFFSET;
  ...
  public static final int NUM_THIN_LOCK_BITS = ADDRESS_BASED_HASHING && !SUBARCH_CLASS_IDX_IN_HEADER ? 22 : 20;
  /** How many bits to shift to get the thin lock? */
  public static final int THIN_LOCK_SHIFT = ADDRESS_BASED_HASHING && !SUBARCH_CLASS_IDX_IN_HEADER ? 10 : 12;
  ...
  @Interruptible
  @NoSubArchCompile
  public static void writeAvailableByteBootImage(BootImageInterface bootImage, Address objectRef, byte val) {
  	bootImage.setByte(objectRef.plus(AVAILABLE_BITS_OFFSET), val);
  }
  ...
  @Interruptible
  @NoSubArchCompile
  public static void baselineEmitLoadTIB(VM_AbstractAssembler asm, int dest, int object, boolean forSubArch) {
  	if (forSubArch) {
  		VM_Configuration.subArchHelper.baselineEmitLoadTIB(asm, dest, object, STATUS_OFFSET);
  	} else {
