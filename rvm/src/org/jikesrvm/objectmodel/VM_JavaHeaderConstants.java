...
public interface VM_JavaHeaderConstants extends VM_SizeConstants {
  ...
  boolean SUBARCH_CLASS_IDX_IN_HEADER = VM_Configuration.SubArchCellSpu;

  /** How many bits in the header are available for the GC and MISC headers? */
  int NUM_AVAILABLE_BITS = SUBARCH_CLASS_IDX_IN_HEADER ? 0 : (ADDRESS_BASED_HASHING ? 8 : 2);
   ...
   */ 
  int ADDR_HASH_SHIFT = SUBARCH_CLASS_IDX_IN_HEADER ? 10 : 8;
  Word HASH_STATE_UNHASHED = Word.zero();
  Word HASH_STATE_HASHED = Word.one().lsh(ADDR_HASH_SHIFT); //0x00000100
  Word HASH_STATE_HASHED_AND_MOVED = Word.fromIntZeroExtend(3).lsh(ADDR_HASH_SHIFT); //0x0000300
