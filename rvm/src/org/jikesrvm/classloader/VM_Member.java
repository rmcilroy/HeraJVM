...
public abstract class VM_Member extends VM_AnnotatedElement implements VM_Constants, VM_ClassLoaderConstants {
  ...
  protected static final int NO_OFFSET = Short.MIN_VALUE + 1;
  ...
  /**
   * Annotation modifiers associated with this member.
   */
  protected final short annoModifiers;
  ...
  /**
   * The member's jtoc/obj/tib offset in bytes for use by sub-arch
   * Set by {@link VM_Class#resolveForSubArch()}
   */
  protected int subArchOffset;
   ...
   */
  protected VM_Member(VM_TypeReference declaringClass, VM_MemberReference memRef, short modifiers,
  										short annoModifiers, VM_Atom signature, VM_Annotation[] annotations) {
    super(annotations);
    ...
    this.subArchOffset = NO_OFFSET; // invalid value. Set to valid value during VM_Class.resolveForSubArch()
  }
  ...
  public final Offset getOffset() {
    if (VM.VerifyAssertions) VM._assert(declaringClass.isLoaded(false));
  ...
  //------------------------------------------------------------------//
  // The following are available after the declaring class has been   //
  // "resolved for subarch".                                          //
  //------------------------------------------------------------------//

  /**
   * Offset of this field or method, in bytes for the subarch
   * <ul>
   * <li> For a static field:      offset of field from start of jtoc
   * <li> For a static method:     offset of code object reference from start of jtoc
   * <li> For a non-static field:  offset of field from start of object
   ...
   */
  @Uninterruptible
  public final Offset getSubArchOffset() {
    if (VM.VerifyAssertions && VM.runningVM) VM._assert(subArchOffset != NO_OFFSET);
    return Offset.fromIntSignExtend(subArchOffset);
  }
   ...
   * and this method doesn't have to be final.
   */
  public final void setSubArchOffset(Offset off) {
    subArchOffset = off.toInt();
  }
}
