...
// TODO !! Sort out interface invocation on subarch
...
public class VM_InterfaceInvocation implements VM_TIBLayoutConstants, VM_SizeConstants {
  ...
  public static VM_CodeArray invokeInterface(Object target, int mid) throws IncompatibleClassChangeError {
    ...
    VM_Method sought = mref.resolveInterfaceMethod(false);
    ...
    if (VM.BuildForITableInterfaceInvocation) {
      ...
      return (ArchitectureSpecific.VM_CodeArray) found.getCurrentEntryCodeArray(false);
    }
  }
  ...
  public static void invokeinterfaceImplementsTest(VM_Class LHSclass, Object[] RHStib)
    ...
    if (!LHSclass.isResolved(false)) {
      LHSclass.resolve(false);
    }
  ...
  public static void unresolvedInvokeinterfaceImplementsTest(int mid, Object[] RHStib)
      throws IncompatibleClassChangeError {
    VM_Method sought = VM_MemberReference.getMemberRef(mid).asMethodReference().resolveInterfaceMethod(false);
  ...
  */
  ...
  private static Object[] buildITable(VM_Class C, VM_Class I) {
    ...
    for (VM_Method im : interfaceMethods) {
      ...
      if (vm.isStatic()) {
        vm.compile(false);
        iTable[getITableIndex(I, im.getName(), im.getDescriptor())] = vm.getCurrentEntryCodeArray(false);
      } else {
  ...
  */
   ...
   */
  public static void updateTIBEntry(VM_Class klass, VM_Method m, boolean forSubArch) {
  	// TODO - Fix for Subarch
  	VM._assert(!forSubArch);
    Object[] tib = klass.getTypeInformationBlock();
    if (VM.BuildForIMTInterfaceInvocation) {
        ...
        for (int i = 0; i < IMT_METHOD_SLOTS; i++) {
          if (map[i] == m) {
            if (VM.BuildForIndirectIMT) {
              ...
              IMT[i] = (ArchitectureSpecific.VM_CodeArray) m.getCurrentEntryCodeArray(false);
            } else {
              tib[i + TIB_FIRST_INTERFACE_METHOD_INDEX] = m.getCurrentEntryCodeArray(false);
            }
          ...
          if (iTable != null) {
            ...
            for (VM_Method im : interfaceMethods) {
              if (im.getName() == name && im.getDescriptor() == desc) {
                iTable[getITableIndex(I, name, desc)] = m.getCurrentEntryCodeArray(false);
              }
            }
          }
        }
  ...
  private static final class IMTDict {
    ...
    public void populateIMT(Object[] tib, Object[] IMT) {
      ...
      for (int slot = 0; slot < links.length; slot++) {
        ...
        if (count == 0) {
          VM_Entrypoints.raiseAbstractMethodError.compile(false);
          IMT[extSlot] = VM_Entrypoints.raiseAbstractMethodError.getCurrentEntryCodeArray(false);
        } else if (count == 1) {
          VM_Method target = getSoleTarget(slot);
          if (target.isStatic()) {
            target.compile(false);
            IMT[extSlot] = target.getCurrentEntryCodeArray(false);
          } else {
