...
// TODO - deal with annotations from subarch
...
public final class VM_Annotation {
  ...
  private Annotation createValue() {
    ...
    VM_Class annotationInterface = VM_TypeReference.findOrCreate(classLoader, type).resolve(false).asClass();
    if (!annotationInterface.isResolved(false)) {
      annotationInterface.resolve(false);
    }
    VM_Class annotationClass = annotationInterface.getAnnotationClass();
    if (!annotationClass.isResolved(false)) {
      annotationClass.resolve(false);
    }
    if (!annotationClass.isInitialized(false)) {
      ...
      case'e': {
        ...
        @SuppressWarnings("unchecked") Class enumType =
            VM_TypeReference.findOrCreate(classLoader,
                                          VM_Class.getUtf(constantPool, typeNameIndex)).resolve(false).getClassForType();
  ...
  static boolean equals(BaseAnnotation a, VM_Annotation vmA, BaseAnnotation b, VM_Annotation vmB) {
    if (vmA.type != vmB.type) {
      ...
      VM_Class annotationInterface = VM_TypeReference.findOrCreate(vmA.classLoader, vmA.type).resolve(false).asClass();
      VM_Class annotationClass = annotationInterface.getAnnotationClass();
      VM_Field[] annotationClassFields = annotationClass.getDeclaredFields();
  ...
  abstract static class BaseAnnotation implements Annotation {
    ...
    public Class<? extends Annotation> annotationType() {
      return (Class<? extends Annotation>) vmAnnotation.annotationType().resolve(false).getClassForType();
    }
