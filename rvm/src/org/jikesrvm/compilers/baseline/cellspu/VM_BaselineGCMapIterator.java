...
public abstract class VM_BaselineGCMapIterator extends VM_GCMapIterator implements VM_BaselineConstants {
  ...
  public void setupIterator(VM_CompiledMethod compiledMethod, Offset instructionOffset, LocalAddress fp) {
    ...
    if (currentMethod.getDeclaringClass().hasDynamicBridgeAnnotation()) {
      ...
      LocalAddress ip = null; // TODO - fix VM_Magic.getNextInstructionAddress(fp);
  ...
  public void reset() {
    ...
    if (bridgeTarget != null) {
      ...
      bridgeRegisterLocation = framePtr.loadLocalAddress();
     //TODO - FIX bridgeRegisterLocation =
     ...
     //                                  BYTES_IN_ADDRESS * (LAST_NONVOLATILE_GPR - FIRST_VOLATILE_GPR + 1));
      ...
      LocalAddress callersFP = VM_Magic.getCallerFramePointer(framePtr);
  ...
  public LocalAddress getNextReferenceAddress() {
    ...
    if (!finishedWithRegularMap) {
      ...
      if (mapIndex != 0) {
        ...
        nextCallerAddress = nextCallerAddress.loadLocalAddress();
        if (VM_Compiler.isRegister(location)) {
          return registerLocations.get(location).toLocalAddress();
        } else {
    ...
    if (bridgeParameterMappingRequired) {
      if (!bridgeRegistersLocationUpdated) {
        ...
        //
        LocalAddress location = LocalAddress.zero(); // FIXME: framePtr.plus(VM_Compiler.getFrameSize(currentCompiledMethod));
        // TODO - fixlocation = location.minus((LAST_NONVOLATILE_FPR - FIRST_VOLATILE_FPR + 1) * BYTES_IN_DOUBLE);
  ...
  public LocalAddress getNextReturnAddressAddress() {
    ...
    if (mapIndex == 0) return LocalAddress.zero();
  ...
  private void updateCallerRegisterLocations() {
    ...
    if (!currentMethod.getDeclaringClass().hasDynamicBridgeAnnotation()) {
      ...
      LocalAddress addr = LocalAddress.zero(); // FIXME framePtr.plus(VM_Compiler.getFrameSize(currentCompiledMethod));
    // TODO - FIX  addr =
