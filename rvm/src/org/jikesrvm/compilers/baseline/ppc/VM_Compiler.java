...
public abstract class VM_Compiler extends VM_BaselineCompiler
    implements VM_BaselineConstants, VM_JNIStackframeLayoutConstants, VM_BBConstants, VM_AssemblerConstants, VM_StackframeLayoutConstants {
  ...
  final ArchitectureSpecific.VM_Assembler asm;
...
//    pushFloat(F0);
  ...
  */
  ...
  protected final void emit_unresolved_invokevirtual(VM_MethodReference methodRef) {
    ...
    VM_ObjectModel.baselineEmitLoadTIB(asm, T1, T0, false); // load TIB
  ...
  protected final void emit_unresolved_invokestatic(VM_MethodReference methodRef) {
  	// TODO - work out annotation modifers for method
  	emitDynamicLinkingSequence(T2, methodRef, true);                  // leaves method offset in T2
  ...
  protected final void emit_resolved_invokestatic(VM_MethodReference methodRef) {
  	VM_Method method = methodRef.peekResolvedMethod(false);
    if (method.isSubArchMigrationPoint()) {
    	// migrate thread to the subarch spu

    	// resolve declaring class for subarch
    	method.getDeclaringClass().resolve(true);
    	method.getDeclaringClass().instantiate(true);
    	
    	// load parameters for syscall
    	asm.emitLVAL(T0, VM_Magic.objectAsAddress(method).toInt());
    	asm.emitADDI(T1, spTopOffset + BYTES_IN_STACKSLOT - BYTES_IN_INT, FP);
    	asm.emitLVAL(T2, methodRef.getParameterWords());
    	genMigrateToSubArchSysCall(methodRef);
    	// pop parameters and push return value
	    genPopParametersAndPushReturnValue(false, methodRef);
    } else {
      // perform normal invocation
    	Offset methodOffset = method.getOffset();
  ...
  protected final void emit_invokeinterface(VM_MethodReference methodRef) {
    ...
    if (VM.BuildForIMTInterfaceInvocation || (VM.BuildForITableInterfaceInvocation && VM.DirectlyIndexedITables)) {
      if (methodRef.isMiranda()) {
        ...
        if (resolvedMethod == null) {
          ...
          VM_ObjectModel.baselineEmitLoadTIB(asm, T1, T1, false);
    ...
    if (VM.BuildForIMTInterfaceInvocation) {
      ...
      if (itableIndex == -1) {
        ...
        asm.emitMTCTR(T0);
        peekAddr(T0, count - 1);     // object
        VM_ObjectModel.baselineEmitLoadTIB(asm, T0, T0, false);
  ...
  private void emitDynamicLinkingSequence(int reg, VM_MemberReference ref, boolean couldBeZero) {
    ...
    if (couldBeZero) {
      ...
      asm.emitLVAL(T1, 0);            		// not for subarch, load false into arg 2
  ...
  private void genMigrateToSubArchSysCall(VM_MethodReference methodRef) {
  	VM_TypeReference t = methodRef.getReturnType();
    Offset migrateMethodOffset;
  	if (t.isVoidType()) {
      migrateMethodOffset = VM_Entrypoints.subArchMigrateMethodReturningVoid.getOffset();
    } else if (t.isLongType()) {
      migrateMethodOffset = VM_Entrypoints.subArchMigrateMethodReturningLong.getOffset();
    } else if (t.isFloatType()) {
      migrateMethodOffset = VM_Entrypoints.subArchMigrateMethodReturningFloat.getOffset();
    } else if (t.isDoubleType()) {
      migrateMethodOffset = VM_Entrypoints.subArchMigrateMethodReturningDouble.getOffset();
    } else if (t.isIntLikeType()) {
      migrateMethodOffset = VM_Entrypoints.subArchMigrateMethodReturningInt.getOffset();
    } else { // t is object
      migrateMethodOffset = VM_Entrypoints.subArchMigrateMethodReturningRef.getOffset();
    }
  	asm.emitLAddrToc(S0, migrateMethodOffset);
    asm.emitMTCTR(S0);
    asm.emitBCCTRL();
	}
  ...
  private boolean generateInlineCode(VM_MethodReference methodToBeCalled) {
    ...
    // Address and LocalAddress are the same on ppc
    if (methodToBeCalled.getType() == VM_TypeReference.Address  || methodToBeCalled.getType() == VM_TypeReference.LocalAddress) {
      // Address.xyz magic
    ...
    if (methodName == VM_MagicNames.getFramePointer) {
      ...
      // return false (PPC is a main arch)
      asm.emitLVAL(T0, 0);
      pushInt(T0);
      ...
      VM_Array type = methodToBeCalled.getType().resolve(false).asArray();
