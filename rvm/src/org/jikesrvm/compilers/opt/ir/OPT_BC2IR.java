...
public final class OPT_BC2IR
  ...
  public OPT_Instruction generateAnewarray(VM_TypeReference elementTypeRef) {
    ...
    if (arrayType != null) {
      if (!(arrayType.isInitialized(false) || arrayType.isInBootImage()) && VM_Type.JavaLangObjectType.isInstantiated(false)) {
        VM_Type elementType = elementTypeRef.peekType();
        if (elementType != null) {
          if (elementType.isInitialized(false) || elementType.isInBootImage()) {
            arrayType.resolve(false);
            arrayType.instantiate(false);
          }
        }
      }
      if (arrayType.isInitialized(false) || arrayType.isInBootImage()) {
        op = NEWARRAY;
    ...
    while (true) {
      ...
      switch (code) {
        ...
        case JBC_getstatic: {
          ...
          boolean unresolved = ref.needsDynamicLink(bcodes.getMethod(), false);
          ...
          if (unresolved) {
            ...
            VM_Field field = ref.peekResolvedField(false);
            ...
            if (field.isFinal()) {
              ...
              if (declaringClass.isInitialized(false) || declaringClass.isInBootImage()) {
                try {
        ...
        case JBC_getfield: {
          ...
          if (unresolved) {
            ...
            field = ref.peekResolvedField(false);
            offsetOp = new OPT_AddressConstantOperand(field.getOffset());
        ...
        case JBC_invokevirtual: {
          ...
          if (ref.isMiranda()) {
            ...
            if (receiverType != null && receiverType.isInitialized(false) && !receiverType.isInterface()) {
            ...
            if (vmeth != null) {
              ...
              boolean unresolved = vmethRef.needsDynamicLink(bcodes.getMethod(), false);
              if (unresolved) {
                OPT_RegisterOperand offsetrop = gc.temps.makeTempOffset();
            ...
            boolean unresolved = ref.needsDynamicLink(bcodes.getMethod(), false);
            VM_Method target = ref.peekResolvedMethod(false);
            ...
            if (type != null && type.isResolved(false) && type.isClassType()) {
        ...
        case JBC_invokespecial: {
          VM_MethodReference ref = bcodes.getMethodReference();
          VM_Method target = ref.resolveInvokeSpecial(false);
        ...
        case JBC_invokeinterface: {
          ...
          if (requiresImplementsTest) {
            if (resolvedMethod == null) {
              ...
              if (receiverType != null && receiverType.isResolved(false) && !receiverType.isInterface()) {
                byte doesImplement =
        ...
        case JBC_new: {
          ...
          if (klassType != null && (klassType.isInitialized(false) || klassType.isInBootImage())) {
  ...
  private boolean couldCauseClassLoading(VM_TypeReference typeRef) {
    ...
    if (type.isInitialized(false)) return false;
    if (type.isArrayType()) return !type.isResolved(false);
