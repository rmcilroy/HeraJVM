...
public abstract class OPT_Simplifier extends OPT_IRTools {
    ...
    if (CF_FIELDS) {
      ...
      if ((methOp != null) && methOp.isVirtual() && !methOp.hasPreciseTarget()) {
        ...
        if (calleeThis.isNullConstant()) {
          ...
          if (calleeClass.isResolved(false)) {
      ...
      if ((methOp != null) && methOp.hasPreciseTarget() && methOp.getTarget().isPure()) {
        ...
        try {
          if (VM.runningVM) {
            ...
            for(int i=0; i < n; i++) {
              argTypes[i] = Call.getParam(s,i).getType().resolve(false).getClassForType();
            }
  ...
  private static DefUseEffect getField(OPT_Instruction s) {
    if (CF_FIELDS) {
      ...
      if (VM.VerifyAssertions && ref.isNullConstant()) {
        ...
        VM_Field field = GetField.getLocation(s).getFieldRef().resolve(false);
        if (field.isFinal() && field.getDeclaringClass().isInitialized(false)) {
          try {
  ...
  private static DefUseEffect getObjTib(OPT_Instruction s) {
    if (CF_TIB) {
      ...
      if (op.isNullConstant()) {
        ...
        if (typeRef.isResolved(false) && rop.isPreciseType() && typeRef.resolve(false).isInstantiated(false)) {
  ...
  private static DefUseEffect getClassTib(OPT_Instruction s) {
    if (CF_TIB) {
      ...
      if (typeOp.getTypeRef().isResolved(false)) {
        Move.mutate(s,
                    REF_MOVE,
  ...
  private static DefUseEffect refLoad(OPT_Instruction s) {
    if (CF_TIB) {
      ...
      if (base.isTIBConstant()) {
        ...
        if (tib.value.isInstantiated(false) && offset.isConstant()) {
