...
public final class VM_OptCompiledMethod extends VM_CompiledMethod {
  ...
  public VM_OptCompiledMethod(int id, VM_Method m, boolean forSubArch) {
  ...
  public void set(VM_StackBrowser browser, Offset instr) {
    ...
    if (iei >= 0) {
      ...
      browser.setMethod(VM_MemberReference.getMemberRef(mid).asMethodReference().peekResolvedMethod(false));
  ...
  public void printStackTrace(Offset instructionOffset, VM_PrintLN out) {
    ...
    if (iei >= 0) {
      ...
      for (int j = iei; j >= 0; j = VM_OptEncodedCallSiteTree.getParent(j, inlineEncoding)) {
        ...
        VM_NormalMethod m =
            (VM_NormalMethod) VM_MemberReference.getMemberRef(mid).asMethodReference().peekResolvedMethod(false);
  ...
  public void applyCodePatches(VM_CompiledMethod cm) {
    if (patchMap != null) {
      for (int idx = 0; idx < patchMap.length; idx += 2) {
        ArchitectureSpecific.VM_CodeArray code = (ArchitectureSpecific.VM_CodeArray) cm.codeArrayForOffset(Offset.fromIntZeroExtend(patchMap[idx]));
        if (VM.BuildForIA32) {
