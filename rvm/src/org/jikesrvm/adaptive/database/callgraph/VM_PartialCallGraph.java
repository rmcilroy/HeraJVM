...
public final class VM_PartialCallGraph implements VM_Decayable, VM_Reportable {
  ...
  public synchronized void dumpGraph(String fn) {
    ...
    for (final VM_CallSite cs : tmp) {
      ...
      ct.visitTargets(new VM_WeightedCallTargets.Visitor() {
        public void visit(VM_Method callee, double weight) {
          VM_CodeArray callerArray = (VM_CodeArray) cs.getMethod().getCurrentEntryCodeArray(false);
          VM_CodeArray calleeArray = (VM_CodeArray) callee.getCurrentEntryCodeArray(false);
          try {
            f.write("CallSite " +
