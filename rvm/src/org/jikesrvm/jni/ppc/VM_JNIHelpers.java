...
public abstract class VM_JNIHelpers extends VM_JNIGenericHelpers
  ...
  public static Object invokeInitializer(Class<?> cls, int methodID, Address argAddress, boolean isJvalue,
    ...
    VM_Method mth = VM_MemberReference.getMemberRef(methodID).asMethodReference().resolve(false);
    ...
    if (isJvalue) {
      ...
      if (isDotDotStyle) {
        if (VM.BuildForPowerOpenABI || VM.BuildForMachOABI) {
          ...
          //
          //      native C method ->
          Address gluefp = VM_Magic.localAddressAsAddress(VM_Magic.getCallerFramePointer(VM_Magic.getCallerFramePointer(VM_Magic.getFramePointer())));
  ...
  private static Address pushVarArgToSpillArea(int methodID, boolean skip4Args) throws Exception {
    if (VM.BuildForPowerOpenABI || VM.BuildForSVR4ABI) {
      ...
      LocalAddress gluefp =
          VM_Magic.getFramePointer().plus(ArchitectureSpecific.VM_ArchConstants.STACKFRAME_FRAME_POINTER_OFFSET).loadLocalAddress();
      gluefp = gluefp.plus(ArchitectureSpecific.VM_ArchConstants.STACKFRAME_FRAME_POINTER_OFFSET).loadLocalAddress();
      gluefp = gluefp.plus(ArchitectureSpecific.VM_ArchConstants.STACKFRAME_FRAME_POINTER_OFFSET).loadLocalAddress();
      ...
      LocalAddress varargAddress = gluefp.plus(spillAreaOffset);
      ...
      return VM_Magic.localAddressAsAddress(varargAddress);


    } else {
      if (VM.VerifyAssertions) VM._assert(VM.BuildForMachOABI);
      // get the FP for this stack frame and traverse 2 frames to get to the glue frame
      LocalAddress currentfp = VM_Magic.getFramePointer();
      LocalAddress gluefp =
          VM_Magic.getFramePointer().plus(ArchitectureSpecific.VM_ArchConstants.STACKFRAME_FRAME_POINTER_OFFSET).loadLocalAddress();
      gluefp = gluefp.plus(ArchitectureSpecific.VM_ArchConstants.STACKFRAME_FRAME_POINTER_OFFSET).loadLocalAddress();
      gluefp = gluefp.plus(ArchitectureSpecific.VM_ArchConstants.STACKFRAME_FRAME_POINTER_OFFSET).loadLocalAddress();
      LocalAddress gluecallerfp =
          gluefp.plus(ArchitectureSpecific.VM_ArchConstants.STACKFRAME_FRAME_POINTER_OFFSET).loadLocalAddress();
      // compute the offset into the spill area of the native caller frame,
      // skipping the args which are not part of the arguments for the target method
      ...
      LocalAddress targetAddress = gluefp.plus(STACKFRAME_HEADER_SIZE + ((skip4Args ? 4 : 3)) * BYTES_IN_ADDRESS);
      ...
      if (argSize > spillRequiredAtOffset) {
        ...
        LocalAddress srcAddress = gluecallerfp.plus(14 * BYTES_IN_ADDRESS);
      ...
      // At this point, all the vararg values should be in the spill area in the caller frame
      // return the address of the beginning of the vararg to use in invoking the target method
      return VM_Magic.localAddressAsAddress(targetAddress);
    }
  }
  ...
  */
  static Object[] packageParameterFromDotArgSVR4(VM_Method targetMethod, Address glueFP, boolean skip4Args) {
    if (VM.BuildForSVR4ABI || VM.BuildForMachOABI) {
      ...
      Address nativeFP = VM_Magic.localAddressAsAddress(VM_Magic.getCallerFramePointer(VM_Magic.addressAsLocalAddress(glueFP)));
      VM_TypeReference[] argTypes = targetMethod.getParameterTypes();
      int argCount = argTypes.length;
