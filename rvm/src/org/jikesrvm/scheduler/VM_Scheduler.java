...
public abstract class VM_Scheduler {
   ...
   */
  @NoSubArchCompile
  public static void dumpStack(LocalAddress fp) {
   ...
   */
  @NoSubArchCompile
  public static void dumpStack(LocalAddress ip, LocalAddress fp) {
    ++inDumpStack;
    if (inDumpStack > 1 &&
   ...
   */
  @NoSubArchCompile
  private static boolean isAddressValidFramePointer(final LocalAddress address) {
    return address.EQ(VM_Magic.addressAsLocalAddress(ArchitectureSpecific.VM_StackframeLayoutConstants.STACKFRAME_SENTINEL_FP)) ||
           MM_Interface.mightBeFP(VM_Magic.localAddressAsAddress(address));
  }

  @NoSubArchCompile
  private static void showPrologue(LocalAddress fp) {
    VM.sysWrite("   at ");
   ...
   */
  @NoSubArchCompile
  private static void showMethod(int compiledMethodId, LocalAddress fp) {
    showPrologue(fp);
   ...
   */
  @NoSubArchCompile
  private static void showMethod(String name, LocalAddress fp) {
    showPrologue(fp);
    VM.sysWrite("<");
    VM.sysWrite(name);
    VM.sysWrite(">\n");
   ...
   */
  @NoSubArchCompile
  private static void showMethod(VM_Method method, int lineNumber, LocalAddress fp) {
    showPrologue(fp);
    if (method == null) {
  ...
  @Entrypoint
  @NoSubArchCompile
  public static void dumpStackAndDie(LocalAddress fp) {
    if (!exitInProgress) {
