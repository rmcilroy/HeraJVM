...
public class VM_GreenThread extends VM_Thread {
   ...
   * isn't set.
   */
  public VM_GreenThread(Thread thread, long stacksize, String name, boolean daemon, int priority, boolean forSubArch) {
    this(MM_Interface.newStack((stacksize <= 0) ? STACK_SIZE_NORMAL : (int)stacksize, false),
        thread, name, daemon, false, priority, forSubArch);
  }

  /**
   * Create a thread.
   */
  protected VM_GreenThread(byte[] stack, Thread thread, String name, boolean daemon, boolean system, int priority, boolean forSubArch) {
    super(stack, thread, name, daemon, system, priority, forSubArch);
    // for load balancing
   ...
   */
  public static void yieldpoint(int whereFrom, LocalAddress yieldpointServiceMethodFP) {
  ...
  /**
   * Put given thread onto the SubArch wait queue.
   * @param waitData the wait data specifying the subarch threadID
   * to wait for
   */
  public static void subArchWaitImpl(VM_ThreadSubArchWaitData waitData) {
    VM_GreenThread myThread = VM_GreenScheduler.getCurrentThread();
    myThread.waitData = waitData;
    myThread.changeThreadState(State.RUNNABLE, State.SUBARCH_WAITING);
    yield(VM_GreenProcessor.getCurrentProcessor().subArchQueue);
    myThread.changeThreadState(State.SUBARCH_WAITING, State.RUNNABLE);
  }
