...
class VM_Wait {
  ...
  public static VM_ThreadSubArchWaitData subArchWait(int threadId) {
  	return subArchWait(threadId, VM_ThreadEventConstants.WAIT_INFINITE);
  }
  
  /**
   * Suspend execution of current thread until thread has migrated back from
   * the subarch
   * @param threadId the subarch thread id we are waiting for
   * @param totalWaitTime the number of seconds to wait; negative values
   *   indicate an infinite wait time
   * @return the wait data object indicating the result of the wait
   */
  public static VM_ThreadSubArchWaitData subArchWait(int threadId, double totalWaitTime) {
    // Create wait data to represent the event the thread is
    // waiting for
    long maxWaitNano = getMaxWaitNano(totalWaitTime);
    VM_ThreadSubArchWaitData waitData = new VM_ThreadSubArchWaitData(maxWaitNano, threadId);
    
    // Put the thread on the ioQueue
    VM_GreenThread.subArchWaitImpl(waitData);
    return waitData;
  }
