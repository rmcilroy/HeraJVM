...
public final class VM_MainThread extends Thread {
  ...
  public void run() {
    ...
    VM_Class cls = null;
    try {
    	// TODO - See if we can swap the main thread to the subarch if it would be preferable
      ...
      VM.sysWrite("main thread run " + mainClass.getName() + "\n");
      
      cls = mainClass.resolve(false).asClass();
      cls.resolve(false);
