...
public abstract class VM_MachineSpecificPowerPC extends VM_MachineSpecific implements VM_ArchConstants {
  ...
  */
   ...
   */
  @Interruptible
  public final void baselineEmitLoadTIB(VM_AbstractAssembler asm_a, int dest, int object, Offset tibOffset) {
    VM_Assembler asm = (VM_Assembler) asm_a;
  ...
  public final void initializeStack(VM_Registers contextRegisters, LocalAddress ip, LocalAddress sp) {
    ...
    fp = VM_Magic.addressAsLocalAddress(VM_Memory.alignDown(VM_Magic.localAddressAsAddress(sp.minus(INITIAL_FRAME_SIZE)), STACKFRAME_ALIGNMENT));
    fp.plus(STACKFRAME_FRAME_POINTER_OFFSET).store(VM_Magic.addressAsLocalAddress(STACKFRAME_SENTINEL_FP));
