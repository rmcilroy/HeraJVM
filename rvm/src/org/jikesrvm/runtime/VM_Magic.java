...
public final class VM_Magic {
  ...
  public static LocalAddress getFramePointer() {
  ...
  /** Get contents of "jtoc" register in the SubArch */
  public static LocalAddress getSubArchJTOC() {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    return null;
  }
  ...
  /** Returns true if running on the sub-arch processor */
  @Uninterruptible
  public static boolean runningOnSubArch() {
   ...
   */
  public static LocalAddress getCallerFramePointer(LocalAddress fp) {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    return null;
  }
   ...
   */
  public static void setCallerFramePointer(LocalAddress fp, LocalAddress newCallerFP) {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
  }
   ...
   */
  public static int getCompiledMethodID(LocalAddress fp) {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    return -1;
  }
   ...
   */
  public static void setCompiledMethodID(LocalAddress fp, int newCMID) {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
  }
   ...
   * @param fp its frame pointer.
   */
  public static LocalAddress getNextInstructionAddress(LocalAddress fp) {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    return null;
  }
   ...
   * @param fp its frame pointer
   */
  public static LocalAddress getReturnAddressLocation(LocalAddress fp) {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    return null;
  }
   ...
   */
  @Uninterruptible
  public static LocalAddress getReturnAddress(LocalAddress fp) {
    return getReturnAddressLocation(fp).loadLocalAddress();
  }
   ...
   */
  @Uninterruptible
  public static void setReturnAddress(LocalAddress fp, LocalAddress v) {
  ...
  //---------------------------------------//
  //      Boot Writer fixup support        //
  //---------------------------------------//
  /** Used by bootwriter to write method sizes to TIB by bootimagewriter */
  public static LinkedList<Object[]> bootImageWriterFixupList = null;


  @NoSubArchCompile
  public static void setBootImageWriterFixupList(LinkedList<Object[]> list) {
  	bootImageWriterFixupList = list;
  }
  
  @NoSubArchCompile
	public static void bootWriterFixup(Object obj, int offset, int value) {
  	if (VM.writingBootImage) {
			Object [] tuple = new Object [3];
			tuple[0] = obj;
			tuple[1] = new Integer(offset);
			tuple[2] = new Integer(value);
			VM_Magic.bootImageWriterFixupList.add(tuple);
  	}
	}
   ...
   */
  @NoSubArchCompile
  public static <T> LocalAddress objectAsLocalAddress(T object) {
    if (VM.runningVM && VM.VerifyAssertions) {
      VM._assert(VM.NOT_REACHED); // call site should have been hijacked by magic in compiler
    }

    if (objectAddressRemapper == null) {
      return LocalAddress.zero();                 // tool isn't interested in remapping
    }

    return objectAddressRemapper.objectAsLocalAddress(object);
  }
   ...
   */
  @NoSubArchCompile
  public static Object localAddressAsObject(LocalAddress address) {
    if (VM.runningVM && VM.VerifyAssertions) {
      VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
    ...
    return objectAddressRemapper.localAddressAsObject(address);
  }
  
  /**
   * Cast bits.
   */
  public static Address localAddressAsAddress(LocalAddress address) {
    if (VM.runningVM && VM.VerifyAssertions) {
      VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
    return null;
   ...
   * Cast bits.
   */
  public static LocalAddress addressAsLocalAddress(Address address) {
    if (VM.runningVM && VM.VerifyAssertions) {
      VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
    return null;
   ...
   */
  public static void returnToNewStack(LocalAddress fp) {
    if (VM.VerifyAssertions) VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
  }
  ...
  /**
   * On Cell SPU, read the IN mailbox.
   */
	@Uninterruptible
  public static int readIntMailBox() {
   ...
   */
	@Uninterruptible
  public static Address readRefMailBox() {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
    return null;
  ...
  /**
   * On Cell SPU, write to OUT mailbox.
   */
	@Uninterruptible
  public static void writeMailBox(int value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }
   ...
   */
	@Uninterruptible
  public static void writeMailBox(float value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }  
   ...
   */
	@Uninterruptible
  public static void writeMailBoxUpperWord(long value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }
	...
	@Uninterruptible
  public static void writeMailBoxLowerWord(long value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }
   ...
   */
	@Uninterruptible
  public static void writeMailBoxUpperWord(double value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }  
   ...
   */
	@Uninterruptible
  public static void writeMailBoxLowerWord(double value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }  
   ...
   */
	@Uninterruptible
  public static void writeMailBox(Address value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }
  ...
  /**
   * On Cell SPU, write to INTERUPT OUT mailbox.
   */
	@Uninterruptible
  public static void writeIntrMailBox(int value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }
   ...
   */
	@Uninterruptible
  public static void writeIntrMailBox(Address value) {
    if (VM.VerifyAssertions) {
      //VM._assert(VM.NOT_REACHED);  // call site should have been hijacked by magic in compiler
    }
  }
  ...
  /**
   * On Cell Spu, caches the statics of a given class
   * 
   * @param methodOffset offset of method in JTOC
   * @return address of entry point to method in local memory if cached already, else null
   */
  public static void cacheClassStatics(int staticTocOffset) {
  	if (VM.VerifyAssertions) {
  		VM._assert(VM.NOT_REACHED);
  	}
  }
