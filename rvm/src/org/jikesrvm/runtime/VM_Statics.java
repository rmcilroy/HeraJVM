...
public class VM_Statics implements VM_Constants {
  ...
  /**
   * Linked list of spare numeric slots caused by aligning static of same class for 
   * subarch caching
   */
  private static final VM_LinkedList<Integer> spareWordSlots = new VM_LinkedList<Integer>();
  private static final VM_LinkedList<Integer> spareDoubleSlots = new VM_LinkedList<Integer>();
  
  /**
   * Linked list of spare numeric slots caused by aligning static of same class for 
   * subarch caching
   */
  private static final VM_LinkedList<Integer> spareReferenceSlots = new VM_LinkedList<Integer>();
  ...
  public static int findOrCreateIntSizeLiteral(int literal) {
    ...
    synchronized (intSizeLiterals) {
      ...
      if (offsetAsInt != null) {
        ...
        Offset newOff = allocateNumericSlot(BYTES_IN_INT, true);
  ...
  public static int findOrCreateLongSizeLiteral(long literal) {
    ...
    synchronized (longSizeLiterals) {
      ...
      if (offsetAsInt != null) {
        ...
        Offset newOff = allocateNumericSlot(BYTES_IN_LONG, true);
  ...
  public static int findOrCreateStringLiteral(VM_Atom literal) throws java.io.UTFDataFormatException {
    ...
    synchronized (stringLiterals) {
      ...
      if (offAsInt != null) {
        ...
        Offset newOff = allocateReferenceSlot(true);
  ...
  public static int findOrCreateClassLiteral(int typeReferenceID) {
  	Class<?> literalAsClass = VM_TypeReference.getTypeRef(typeReferenceID).resolve(false).getClassForType();
    Integer offAsInt;
    synchronized (objectLiterals) {
  ...
  /**
   * Aligns numeric slots and returns their offsets to allow
   * all statics in a class to be cached in one go by the subarch
   * 
   * @return
   */
  public static synchronized Offset beginNextClassNumeric() {
  	// align to 16 bytes
  	if (numericSlotHole != middleOfTable) {
  		spareWordSlots.add(numericSlotHole);
      numericSlotHole = middleOfTable;
    }
  	int spareSlots = (nextNumericSlot & 3);
  	if (spareSlots != 3) {
  		for (int i = 0; i <= spareSlots; i++) {
  			if ((nextNumericSlot & 3) == 1) {
  				spareDoubleSlots.add(nextNumericSlot - 1);
  				nextNumericSlot-=2;
  				i++;
  			} else {
  				spareWordSlots.add(nextNumericSlot);
  				nextNumericSlot--;
  			}
  		}
  	}
  	return slotAsOffset(nextNumericSlot + 1);
  }
  
  public static synchronized Offset beginNextClassReference() {
  	// align to 16 bytes
  	int spareSlots = (nextReferenceSlot & 3);
  	if (spareSlots != 0) {
  		spareSlots = 4 - spareSlots;
  		for (int i = 0; i < spareSlots; i++) {
  			spareReferenceSlots.add(nextReferenceSlot);
  			nextReferenceSlot++;
  		}
  	}
  	return slotAsOffset(nextReferenceSlot);
  }
   ...
   */
  public static synchronized Offset allocateNumericSlot(int size, boolean useSpareSlots) {
    // Result slot
    int slot;
    ...
    if (size == BYTES_IN_LONG) {
    	if (useSpareSlots && !spareDoubleSlots.isEmpty()) {
    		// get a spare double slot
    		slot = spareDoubleSlots.remove(0);
    	} else {
	      // widen for a wide
	      nextNumericSlot--;
	      // check alignment
    	...
    	if (useSpareSlots && (!spareWordSlots.isEmpty() || !spareDoubleSlots.isEmpty())) {
    		if (spareWordSlots.isEmpty()) {
      		slot = spareDoubleSlots.remove(0);
      		spareWordSlots.add(slot + 1);
    		} else {
      		slot = spareWordSlots.remove(0);
    		}
    	} else {
   ...
   */
  public static synchronized Offset allocateReferenceSlot(boolean useSpareSlots) {
  	int slot;
  	if (useSpareSlots && !spareReferenceSlots.isEmpty()) {
  		slot = spareReferenceSlots.remove(0);
  	} else {
	    slot = nextReferenceSlot;
  ...
  public static synchronized Offset moveRefToEnd(Offset oldSlot) {
  	Offset newSlot = allocateReferenceSlot(false);
  	setSlotContents(newSlot, getSlotContentsAsObject(oldSlot));
  	spareReferenceSlots.add(offsetAsSlot(oldSlot));
  	return newSlot;  	
  }
